...
def create_policy_model(input_dim, output_dim, learning_rate=0.0001):
    model = Sequential()
    model.add(Dense(8, input_dim=input_dim, activation='relu'))  # Reduced neurons
    model.add(BatchNormalization())  # Added Batch Normalization
    model.add(Dense(8, activation='relu'))  # Reduced neurons
    model.add(BatchNormalization())  # Added Batch Normalization
...
def train_policy_gradient(env, num_episodes, batch_size=32, learning_rate=0.0001):
    ...
    for episode in range(num_episodes):
        ...
        if len(batch_states) >= batch_size:
            ...
            # Train on the batch with early stopping
            early_stopping = EarlyStopping(monitor='loss', patience=3, verbose=1, restore_best_weights=True)
            policy_model.fit(all_states, all_actions_one_hot, sample_weight=sample_weights, 
