...
def create_policy_model(input_dim, output_dim, learning_rate):
    model = Sequential()
    model.add(Dense(32, input_dim=input_dim, activation='tanh'))  # first hidden layer
    model.add(BatchNormalization())  # Added Batch Normalization
    model.add(Dense(32, activation='leaky_relu'))  # second hidden layer
    model.add(BatchNormalization())  # Added Batch Normalization
    model.add(Dense(output_dim, activation='softmax'))  # Softmax output
...
def train_policy_gradient(env, num_episodes, batch_size, learning_rate):
    ...
    early_stopping_patience = 3  # Set patience for early stopping
    best_loss = float('inf')
    ...
    for episode in range(num_episodes):
        ...
        if len(batch_states) >= batch_size:
            ...
            # Train on the batch with train_on_batch
            loss = policy_model.train_on_batch(all_states, all_actions_one_hot, sample_weight=sample_weights)
